...
2. **ブループリントエディタを開く**
   ...
   > **「データのみのブループリント」**という 1 画面だけの簡易表示が出た場合は、
   > 上部の **[フルブループリントエディタを開く]** を押してください。
   > 以降の手順で使う **コンポーネント** パネルは簡易表示では出ません。
...
7. **[コンパイル]** → 保存 → `BP_HapbeatKeyTest` をレベルにドラッグ&ドロップ

8. ▶ Play して **G** を押す → 100Hz が 1 回鳴る
