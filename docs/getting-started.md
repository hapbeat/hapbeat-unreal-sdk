...
音声出力の遅延は、PC の出力経路や Bluetooth ヘッドホンなどの環境で変わります。触覚は約 10ms で届くため、音の出力が遅い環境では**触覚のほうが先に来る**ことがあります。これは想定内です。

1. **Tools → Hapbeat → Hapbeat Settings > Behavior** を開きます。
2. **Haptic Delay Seconds** を `0` から少しずつ上げます。
3. 音と触覚がそろう値で止めます。以後、そのプロジェクトのすべての触覚に同じ遅延が加わります。

この値は PIE 中でも、**次に開始する再生**から反映されます。すでに遅延待ちになっている再生は再計算されません。

実際の遅延量は **Haptic Delay Seconds ＋ entry ごとの `Delay Offset Seconds`**（0 未満は 0 に丸め）です。`Delay Offset Seconds` は、特定のイベントだけを個別に前後させたいときに使います。どちらも発火と停止に同じだけ効くため、イベント自体の長さは変わりません。
...
- **鳴らす場所（起点）** — `Play Event (Hapbeat)` の呼び出し（Blueprint ノード / C++）
...
| **`Gain`** | **0.0〜2.0** |
| **`Pan`** | **-1 = 左 / 0 = 中央 / +1 = 右。このイベントの既定の左右バランス。ノードの `Pan` は**この値に加算**される（合計は ±1 に丸め）。FIRE は対応版ファームウェアが必要（未対応は中央で再生）。CLIP は**モノラルのクリップでも**、再生開始時のパンが 0 でなければステレオにアップミックスして左右に振る** |
...
**再生の経路は 1 本です。** どのエントリを鳴らすかは `Play Event (Hapbeat)`
...
5. **`Play Event (Hapbeat)` を置く**
   グラフの何も無いところで右クリック → 検索欄に **`Play Event (Hapbeat)`** と
...
6. **鳴らすエントリを選ぶ**
   ...
   > 呼び出しごとに強さ・パン・遅延を変えたい場合は、**ノードの ▼ を開く**と
   > `Gain Multiplier` / `Pan` / `Delay Seconds` が現れます（[実行時に変えられるもの](#実行時に変えられるもの)）。

7. **G の `Pressed`** 実行ピンを `Play Event (Hapbeat)` の実行ピンへ接続する
...
停止は **`Stop Event (Hapbeat)`** に同じ `Map` と `Entry` を渡します。
...
エントリの設定（§3）は「既定値」です。呼び出し側からは次の 5 つを実行時に動かせます。

| 変えたいもの | 手段 | 対象 |
|---|---|---|
| 呼び出しごとの強さ | ノードの **`Gain Multiplier`** ピン（詳細ピン。C++ は第 4 引数） | FIRE / CLIP |
| 左右のバランス（パン） | エントリの **`Pan`**（作成時の既定）＋ ノードの **`Pan`** ピン（詳細ピン。**加算**）＋ 再生中は **戻り値の `Set Stream Pan (Hapbeat)`** | 作成時・呼び出し時は FIRE / CLIP、再生中は **CLIP のみ** |
| 再生中の強さ・この再生だけの停止 | **ノードの戻り値**（`Apply Stream Gain Modulation (Hapbeat)` / `Stop Stream Playback (Hapbeat)`） | **CLIP のみ** |
| 遅延 | §1 の **`Haptic Delay Seconds`** ＋ エントリの **`Delay Offset Seconds`** ＋ ノードの **`Delay Seconds`**（詳細ピン）。合計が 0 未満なら 0 | FIRE / CLIP |
| 送信先 | エントリの `Target`（作成時）＋ 実行時は **`Set Address Override`**（[応用](./advanced.md#複数の-hmd-に-1-台ずつ-hapbeat-を割り当てる)） | FIRE / CLIP |

ノードに常時出ている入力ピンは `Map` と `Entry` の 2 つだけ。
**ノードの ▼ を開くと `Gain Multiplier` / `Pan` / `Delay Seconds`** が現れる（詳細ピン）。
呼び出しごとに値を変えるときだけ開けばよい。

`Pan` は -1 = 左 / 0 = 中央 / +1 = 右。ゲインが掛け算なのに対し、**パンは足し算**で、
エントリの `Pan`（そのイベントが普段どこで鳴るか）にノードの `Pan`（この呼び出しでは
どこで起きたか）を足し、±1 に丸めた値が使われる。FIRE ではデバイス側が左右のゲインに
展開し、CLIP では SDK がステレオのサンプルへ事前に掛ける。**FIRE のパンにはデバイス
ファームウェアの対応版が必要**で、未対応のファームウェアは中央で再生する
（更新は [Hapbeat Studio](https://studio.hapbeat.com/) から）。

CLIP は**モノラルのクリップでも**パンできる。再生開始の時点でパンが 0 でなければ、
SDK がステレオへアップミックスして送る。ただし**チャンネル数は再生開始時に確定する**ため、
中央（パン 0）で始めたモノラルの再生に、あとから `Set Stream Pan (Hapbeat)` を掛けても効かない。
モノラルを振るなら、エントリの `Pan` かノードの `Pan` で**再生開始前に**値を渡すこと。

`Delay Seconds` は音の遅延に合わせるための**この呼び出しだけの上乗せ**で、詳細ピン
（ノード下部の ▽）に隠れている。ゲーム的なタイミング調整には UE 標準の `Delay` ノードを使う。
...
> **パンの合成**
> `Pan` はゲインと同じく、EventMap 側の作成時の値と呼び出し時の値の 2 段構えです。
> ゲインが掛け算なのに対しパンは加算で、両方 0 なら中央のままです。
> 再生中に動かす場合は、戻り値の `Set Stream Pan (Hapbeat)` と
...
（`OnComponentHit` など）に `Play Event (Hapbeat)` を繋いでも同じことはできます。
