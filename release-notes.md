...
- サンプルプロジェクトで Basic Example、Showcase、VR Config Example の
  Editor/PIE 動作を確認済み。
- Unreal Engine のコンパイルツールチェーンはこのリポジトリの CI にはないため、
  自動コンパイルは実行していない。配布前に確認すべき箇所は `HapbeatSDK.Build.cs`、
  `HapbeatNetInterfaces.cpp`、エディタ送信側の PONG 回収経路である。
