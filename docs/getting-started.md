...
エントリの設定（§3）は「既定値」です。呼び出し側からは次の 3 つを実行時に動かせます。

| 変えたいもの | 手段 | 対象 |
|---|---|---|
| 呼び出しごとの強さ | ノードの **`Gain Multiplier`** ピン（C++ は第 4 引数） | FIRE / CLIP |
| 再生中の強さ・パン・この再生だけの停止 | **ノードの戻り値**（`Apply Gain Modulation` / `Set Pan` / `Stop`） | **CLIP のみ** |
| 送信先 | エントリの `Target`（作成時）＋ 実行時は **`Set Address Override`**（[応用](./advanced.md#複数の-hmd-に-1-台ずつ-hapbeat-を割り当てる)） | FIRE / CLIP |

**FIRE は送信したあと何も変えられません。** PLAY パケットが運ぶのは
イベント ID・送信先・再生時刻・ゲインの 4 つだけで、再生自体はデバイス内で完結するためです。
鳴っている最中に強弱を付けたい場合は CLIP を使ってください。

> **パンがエントリ（EventMap）側に無いのはなぜか**
> ワイヤに pan フィールドが無く、FIRE では送りようがありません。CLIP でもパンは
> SDK 側がステレオのサンプルへ事前に掛けている処理なので、モノラルのクリップには効きません。
> つまりパンは「作成時に決める値」ではなく「再生中に動かす値」なので、
> 戻り値の `Set Pan` と `Hapbeat Parameter Binding`（[4-3](#4-3-パターン部品)）側に置いています。
