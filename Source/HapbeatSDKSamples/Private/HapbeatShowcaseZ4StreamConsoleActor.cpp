...
void AHapbeatShowcaseZ4StreamConsoleActor::HandleToggleKey()
{
	...
	{
		// The SDK itself selects an exact unicast endpoint after PONG discovery,
		// and broadcasts only while none is known. The sample must use that same
		// policy instead of withholding the user's first press while it waits.
		LoopTrigger->Fire();
	}
}
...
void AHapbeatShowcaseZ4StreamConsoleActor::Tick(float DeltaSeconds)
{
	...
	FHapbeatSampleLibrary::ShowHudLine(StatusHudLineKey,
		...
		bStreaming ? FColor::Green : FColor::Silver,
