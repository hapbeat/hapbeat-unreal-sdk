...
| 左右のバランス（パン） | エントリの **`Pan`**（作成時の既定）＋ ノードの **`Pan`** ピン（詳細ピン。**加算**）＋ 再生中は **戻り値の `Set Pan`** | 作成時・呼び出し時は FIRE / CLIP、再生中は **CLIP のみ** |
...
中央（パン 0）で始めたモノラルの再生に、あとから `Set Pan` を掛けても効かない。
